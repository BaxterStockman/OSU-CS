...
int main(const int argc, char **argv)
{
    ...
    mult_div_values** op_table = create_table(rows, cols);

    // set .mult and .div values
    set_mult_values(op_table, rows, cols);
    set_div_values(op_table, rows, cols);

    cout << endl << "A multiplication table of " << rows << " rows by " << cols << " columns:" << endl << endl;

    for(int iRow=0; iRow < rows; iRow ++)
    {
        for(int iCol=0; iCol < cols; iCol ++)
            cout << op_table[iRow][iCol].mult << '\t';
        cout << endl;
    }

    cout << endl << endl << "A division table of " << rows << " rows by " << cols << " columns:" << endl << endl;

    for(int iRow=0; iRow < rows; iRow ++)
    {
        for(int iCol=0; iCol < cols; iCol ++)
            cout << fixed << setprecision(2) << op_table[iRow][iCol].div << '\t';
        cout << endl;
    }

    cout << endl;

    delete_table(op_table, rows, cols);
