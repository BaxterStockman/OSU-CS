...
//  Date Created:               07/29/2013 06:49:50 PM
//  Last Modification Date:     07/29/2013 11:18:30 PM
//  Filename:                   new_string_fun.cpp
//
//  Overview:
...
void read_command_args(string &str, string usage, int index, int argNum, char **args);

void read_command_args(string &str, string usage);
...
int main(int argc, char *argv[])
{
    ...
    if(argc > 1)
    {
        for(int index = 1; index < argc; index++)   // loop through argv[]
        {
            ...
            {
                read_command_args(initStr, usage);
                revStr = initStr;
...
void read_command_args(string &str, string usage, int index, int argNum, char **args)
{
    // make sure that what follows flag isn't another flag or out-of-range of args[]
    if(index+1 < argNum && (args[index+1][0] != '-' || strlen(args[index+1]) > 3))
    {
        // for all in args[] after flag, add to str if not another flag or out-of-range
        // of args[]
        for(int nextArg = index+1; nextArg < argNum; nextArg++)
        {
            if(nextArg+1 != argNum && (args[nextArg+1][0] != '-' || strlen(args[nextArg+1]) > 3))
            {
                str.insert(str.size(), args[nextArg]);
                str.insert(str.size(), " ");
            }
            else
            {
...
void read_command_args(string &str, string usage)
{
    read_command_args(str, usage, 1, argNum, args);
}
