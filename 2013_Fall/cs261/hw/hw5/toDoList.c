...
/* Print error message and exit program
    ...
    param:  message - C string containing error message
    post:   message has printed to output
...
Task* createTask (int priority, char *desc) {
    ...
    if(!newTask) die("Failed to allocate memory");
    // copy desc into newTask->description; die upon failure
    if(!strcpy(newTask->description, desc)) die("Failed to copy string");
...
void loadList(DynArr *heap, FILE *filePtr) {
    ...
    if(!feof(filePtr)) die("File read error");;
}
...
void printList(DynArr *heap) {
    // create temporary list, which gets sorted, so that
    // list passed as argument doesn't have to be sorted
    // (in case that's a concern for some reason)
...
int compare(TYPE left, TYPE right) {
    struct Task *lhs = (struct Task*)left;
    struct Task *rhs = (struct Task*)right;

    if(lhs->priority > rhs->priority) return 1;
    else if(lhs->priority < rhs->priority) return -1;
    else return 0;
}

/*Define this function, type casting the value of void * to the desired type*/
void print_type(TYPE curval) {
    Task* data1 = (Task*)curval;
