...
int my_compar(const void *left, const void *right) {
    char *l_str = (char*)left;
    char *r_str = (char*)right;

    return strcmp(l_str, r_str);
}

void my_print(const void *value, int (*print_t)(const char *, ...)) {
    print_t("%s", *(char*)value);
}
...
int main(void) {
    ...
    if(openHashTableBagContains(ht, cat, my_compar))
        printf("Table contains cat.\n");
    if(openHashTableBagContains(ht, dog, my_compar))
        printf("Table contains dog.\n");
