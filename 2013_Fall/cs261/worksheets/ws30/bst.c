...
void BSTIteratorInit(struct BSTree * tree, struct BSTIterator *itr) {
    itr = malloc(sizeof(struct BSTIterator));
    itr->stk = createDynArr(ITR_STK_INIT_SZ);
    itr->tree = tree;
}
...
int BSTIteratorHasNext(struct BSTree *tree, struct BSTIterator *itr) {
    if(itr->stk == NULL)
        _slideLeft(tree->root, itr);
    else {
        struct Node *n = (struct Node *)topDynArr(itr->stk);
        popDynArr(itr->stk);
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    //else if (d_compare(val, cur->val) < 0)
...
int containsBSTree(struct BSTree *tree, TYPE val) {
    ...
    while(cur) {
        comp = compare(cur->val, val);
        // return 1 if we've found val
        if(comp == 0) return 1;
        // head left if val is less than cur node's value
        if(comp == 1) {
            cur = cur->left;
            continue;
        }
...
TYPE _leftMost(struct Node *cur) {
    ...
    if(cur->left) return _leftMost(cur->left);
    return cur->val;
}
...
struct Node *_removeNode(struct Node *cur, TYPE val) {
    // make sure we've been passed a valid argument
    assert(cur);
    //int comp = d_compare(cur->val, val);
    int comp = compare(cur->val, val);
    // if we've found val...
    if(comp == 0) {
    ...
    // if val is less than the val of cur, recurse on cur's left leaf.
    else if(comp == 1) cur->left = _removeNode(cur->left, val);
    // otherwise, recurse on cur's left leaf.
    else if(comp == -1) cur->right = _removeNode(cur->right, val);
    return cur;
}
