...
#include "toDoList.h"           // GET RID OF THIS!!!
...
static inline int _ceil(int x, int y) {
...
void _initDynArr(DynArr *v, int capacity) {
    assert(capacity > 0);   // make sure that we're not initializing zero-length array
    assert(v != 0);         // make sure we've been passed a valid pointer
    v->data = malloc(sizeof(TYPE) * capacity);
    assert(v->data != 0);   // check allocation worked
    v->size = 0;            // set array's size to 0
    v->capacity = capacity; // set array's capacity
...
DynArr* createDynArr(int cap) {
    DynArr *r;
    assert(cap > 0);    // check that we're not creating a zero-length array
    r = malloc(sizeof( DynArr));
    assert(r != 0);     // check that allocation worked
    _initDynArr(r,cap);
    return r;
}
...
int sizeDynArr(DynArr *v) {
    assert(v!=0);   // make sure we've got a valid pointer
    return v->size; // return array's size
}
...
*/
int _smallerIndexHeap(DynArr *heap, int i, int j) {
    // check that i and j represent in-bounds indices
    assert((i < heap->size && j < heap->size));
    // if data at i is greater than or equal to data at j,
    // return j; if data at j is greater than i, return i.
    return compare(heap->data[i], heap->data[j]) > 0 ? j : i;
}
...
TYPE getMinHeap(DynArr *heap) {
    // lowest-priority node should be at index 0
    return heap->data[0];
}

/*  Helper function for addHeap() -- recursively
 *  'percolates' added node up the heap into its
 *  proper location

    param:  heap    pointer to the heap
    param:  pos     index of newly-added node
    post:   node is in proper location
*/
void _percUp(DynArr *heap, int pos) {
    // if there's no parent node, bail out
    if(pos < 1) return;
    int parent = (pos-1) / 2;

    // If the value at the parent node is 'greater' (by whatever
    // definition of 'greater' is imposed by compare()) than the
    // value at the child node, swap the values at the parent and,
    // then recurse on the parent node until its parent is a smaller
    // value.
    if(compare(heap->data[parent], heap->data[pos]) == 1) {
...
void addHeap(DynArr *heap, TYPE val) {
    ...
    addDynArr(heap, val);
    // percolate node up to proper position
    _percUp(heap, heap->size - 1);
}
...
void _adjustHeap(DynArr *heap, int max, int pos) {
    ...
    if(rightChild < max) {
        ...
        // the heap property when swapping a child and parent node
